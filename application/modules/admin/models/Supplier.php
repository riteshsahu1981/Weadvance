...
class Admin_Model_Supplier {
    ...
    public function getQuickSupplier()
    {
        return $this->_quickSupplier;
    }
 
    public function setQuickSupplier($quickSupplier)
    {
        $this->_quickSupplier = (string) $quickSupplier;
        return $this;
    }
    ...
    public function setModel($row)
    {
      ...
      $model->setId($row->id)
            ...
            ->setQuickSupplier($row->quick_supplier)
            					;
        ...
        $usersNs = new Zend_Session_Namespace("members");  
     	$data = array(
        	...
        	'last_name' 		=> $this->getLastName(),
                ...
                'quick_supplier'	=> $this->getQuickSupplier(),
                'state'                 => $this->getState(),
                'zip'                   => $this->getZip(),
     		'phone'                 => $this->getPhone(),
                'fax'                   => $this->getFax(),
    ...
    public function getAllSuppliers($status="")
    {
            
        $obj=new Admin_Model_Supplier();
        if($status=="")
            $entries=$obj->fetchAll("id != '-2147483648'" , "org_name");
        else
             $entries=$obj->fetchAll("id != '-2147483648' AND status='1'", "org_name");
        $arrUserLevel=array(''=>"Select");
        foreach($entries as $entry)
        { 	
            $arrUserLevel[$entry->getId()]=$entry->getOrgName() ;
        }
        return $arrUserLevel;	
    }
    ...
    public function quickSupplier(){
        ...
        $entries=$obj->fetchAll("quick_supplier = '1'" , "username");
