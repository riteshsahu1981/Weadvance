<?php
class Security_SysConfigController extends Base_Controller_Action
{
    ...
    { 
        ...
        $select = $table->select()->setIntegrityCheck(false)->from('sys_config')
    ...
    public function addConfigAction()
    {
        ...
        $form    = new Security_Form_SysConfig();
        ...
        if ($request->isPost())
        {
            ...
            { 
                ...
                $model=new Security_Model_SysConfig($options);
                $id=$model->save();
                if($id)  
                {    
                    
                    $this->_flashMessenger->addMessage(array('success'=>'Configuration added successfully!'));
                    $this->_helper->_redirector->gotoUrl($this->view->seoUrl('/security/sys-config/add-config'));  
                }
                else
                {
