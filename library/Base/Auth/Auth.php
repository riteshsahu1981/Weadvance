...
class Base_Auth_Auth extends Zend_Auth  
{
	...
	public function checkUserActiveStatus($options,$identityColumn)
	{
    	...
    	$RES=$user->fetchRow("{$identityColumn}='{$options['email']}' and status ='1'");
    	...
    	if($RES!=false) // if user is not blocked
    	{
    		...
    		$usersNs->userUsername=$RES->getUsername();
                $usersNs->userGroupId=$RES->getGroupId();
                $usersNs->userSubGroupId=$RES->getSubGroupId();
                $usersNs->userRoleId=$RES->getRoleId();
...
//    		}
    ...
    {
      	if ($length < 4 || $length > 44) return false;
